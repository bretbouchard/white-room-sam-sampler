...
#endif
{
    ...
    // Try to load test SoundFont from local development directory
    // Note: This is for development testing only
    // Production builds should bundle SoundFonts in the plugin Resources
...
void SamSamplerPluginProcessor::loadDefaultSoundFont() {
    // Try multiple paths for SoundFont files (development only)
    // Priority order:
    // 1. Local development directory (sf2_local/)
    // 2. Plugin Resources directory (production)
    // 3. Current working directory (fallback)

    juce::File sf2File;

    // Try local development directory first (for testing)
    juce::File localDir = juce::File::getCurrentWorkingDirectory().getChildFile("juce_backend/instruments/Sam_sampler/sf2_local");
    if (localDir.exists()) {
        sf2File = localDir.getChildFile("test_drums.sf2");
        if (sf2File.exists()) {
    ...
    // SoundFonts are copied to Resources/sf2/ by build system
    juce::File resourcesDir = juce::File::getSpecialLocation(juce::File::SpecialLocationType::currentExecutableFile)
                                  .getParentDirectory()
                                  .getChildFile("Resources")
                                  .getChildFile("sf2");

    if (resourcesDir.exists()) {
        juce::Array<juce::File> sf2Files;
        resourcesDir.findChildFiles(sf2Files, juce::File::findFiles, false, "*.sf2");

        if (!sf2Files.isEmpty()) {
            // Prefer piano, fallback to drums
            juce::File preferredFile;

            // Look for piano first
            for (auto& f : sf2Files) {
                if (f.getFileName().containsIgnoreCase("piano") ||
                    f.getFileName().containsIgnoreCase("salamander")) {
                    preferredFile = f;
                    break;
                }
            }

            // If no piano, use first file (likely drums)
            if (!preferredFile.exists()) {
                preferredFile = sf2Files[0];
            }

            DBG("Sam Sampler: Loading SoundFont from Resources: " + preferredFile.getFileName());
            if (samSampler.loadSoundFont(preferredFile.getFullPathName().toUTF8())) {
                DBG("Sam Sampler: Successfully loaded " + preferredFile.getFileName());
                return;
            }
        }
    }
    ...
    sf2File = juce::File::getCurrentWorkingDirectory().getChildFile("test_drums.sf2");
    if (sf2File.exists()) {
        DBG("Sam Sampler: Loading SoundFont from working directory");
        if (samSampler.loadSoundFont(sf2File.getFullPathName().toUTF8())) {
            DBG("Sam Sampler: Successfully loaded test_drums.sf2");
            return;
        }
